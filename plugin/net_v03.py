  ...
  for i in procs:
    try:
      ...
      if ninfo[0] == 'tcp':
        ...
        elif ninfo[3] == '01' or ninfo[3] == '02' or ninfo[3] == '03' or ninfo[3] == '04' or ninfo[3] == '05' or ninfo[
          3] == '06':
          tcpinfo['OTHERS'].append(
            [ninfo[1].split(':')[0], ninfo[1].split(':')[1], ninfo[1], ninfo[2].split(':')[0], ninfo[2].split(':')[1],
             i[1]])
      if ninfo[0] == 'udp':
        if ninfo[3] == '07':
          udpinfo['LISTEN'].append([ninfo[1].split(':')[0], ninfo[1].split(':')[1], ninfo[1], '-', '-', i[1]])
        elif ninfo[3] == '01':
          udpinfo['OTHERS'].append(
            [ninfo[1].split(':')[0], ninfo[1].split(':')[1], ninfo[1], ninfo[2].split(':')[0], ninfo[2].split(':')[1],
             i[1]])
    except Exception, e:
      pass
  ...
  if tflag:
    for ele in ['LISTEN', 'SERVER', 'CLIENT']:
      for row in unique(tcpinfo[ele]):
        if ele == 'LISTEN':
          if row[0] != '00000000':
            print "TCP||" + ele + "||" + read_ip(row[0]) + "||" + read_port(row[1]) + "||" + row[3] + "||" + row[
              4] + "||" + row[5]
        else:
          print "TCP||" + ele + "||" + read_ip(row[0]) + "||" + read_port(row[1]) + "||" + read_ip(
            row[2]) + "||" + read_port(row[3]) + "||" + row[4]
  if uflag:
    for ele in ['LISTEN', 'SERVER', 'CLIENT']:
      for row in unique(udpinfo[ele]):
        if ele == 'LISTEN':
          if row[0] != '00000000':
            print "UDP||" + ele + "||" + read_ip(row[0]) + "||" + read_port(row[1]) + "||" + row[3] + "||" + row[
              4] + "||" + row[5]
        else:
          print "UDP||" + ele + "||" + read_ip(row[0]) + "||" + read_port(row[1]) + "||" + read_ip(
            row[2]) + "||" + read_port(row[3]) + "||" + row[4]
...
if __name__ == '__main__':
  ...
  c.read("/home/opvis/opvis_agent/agent_service/conf.ini")
  url_base = c.get("proxy", "proxyAddress")
  cycle = c.get("proxy", "cycle")
  for item in allip:
